...
static void global(void *data,
	...
	log_d_c_s("global"); log_d_c("%d", name); log_d_c_e("%s", interface);

	if (strcmp(interface, wl_seat_interface.name) == 0) {

		displ->wl_seat = wl_registry_bind(wl_registry, name, &wl_seat_interface, version);
		log_d_c_s("bind"); log_d_c("%d", name); log_d_c("wl_seat"); log_d_c_e("%p", (void*)displ->wl_seat);
		...
		if (version <= RIVER_LAYOUT_V3_VERSION) {
			...
			log_d_c_s("bind"); log_d_c("%d", name); log_d_c("river_layout_manager"); log_d_c_e("%p", (void*)displ->river_layout_manager);
		} else {
			log_f("Invalid river_layout_manager_v3_interface version %d expected %d, exiting", version, RIVER_LAYOUT_V3_VERSION);
			return;
		}
		...
		log_d_c_s("bind"); log_d_c("%d", name); log_d_c("river_control"); log_d_c_e("%p", (void*)displ->river_control);
		...
		log_d_c_s("bind"); log_d_c("%d", name); log_d_c("river_status_manager"); log_d_c_e("%p", (void*)displ->river_status_manager);
		...
		log_d_c_s("bind"); log_d_c("%d", name); log_d_c("wl_output"); log_d_c_e("%p", (void*)wl_output);
