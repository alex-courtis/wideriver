...
static void global(void *data,
	...
	log_debug("global %d %s (v%d)", name, interface, version);

	if (strcmp(interface, wl_seat_interface.name) == 0) {

		displ->wl_seat = wl_registry_bind(wl_registry, name, &wl_seat_interface, version);
		log_debug_c_s("bind:"); log_debug_c("displ:"); log_debug_c("wl_seat"); log_debug_c("%d", name); log_debug_c_e("%p", displ->wl_seat);
		...
		if (version <= RIVER_LAYOUT_V3_VERSION) {
			...
			log_debug_c_s("bind:"); log_debug_c("displ:"); log_debug_c("river_layout_manager"); log_debug_c("%d", name); log_debug_c_e("%p", displ->river_layout_manager);
		} else {
		...
		log_debug_c_s("bind:"); log_debug_c("displ:"); log_debug_c("river_control"); log_debug_c("%d", name); log_debug_c_e("%p", displ->river_control);
		...
		log_debug_c_s("bind:"); log_debug_c("displ:"); log_debug_c("river_status_manager"); log_debug_c("%d", name); log_debug_c_e("%p", displ->river_status_manager);
		...
		log_debug_c_s("bind:"); log_debug_c("displ:"); log_debug_c("wl_output"); log_debug_c("%d", name); log_debug_c_e("%p", wl_output);
...
static void global_remove(void *data,
	...
	log_debug("global_remove %d", name);
